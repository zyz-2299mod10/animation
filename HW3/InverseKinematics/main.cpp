...
int main() {
 ...
 /*       acclaim::Bone x_axis;
        x_axis.start_position = ball.getCurrentPosition();
        x_axis.end_position = ball.getCurrentPosition() + Eigen::Vector4d(10.0, 0.0, 0.0, 0.0);
        graphics::Cylinder axis_graphics;

        axis_graphics.setTexture(Eigen::Vector4f(1.0f, 0.0f, 0.0f, 1.0f));
        Eigen::Vector4d rotaion_axis = Eigen::Vector4d::UnitZ().cross3(x_axis.dir);
        double dot_val = Eigen::Vector4d::UnitZ().dot(x_axis.dir);
        double cross_val = rotaion_axis.norm();
        rotaion_axis.normalize();
        double theta = atan2(cross_val, dot_val);
        x_axis.global_facing = Eigen::AngleAxisd(theta, rotaion_axis.head<3>());
        x_axis.global_facing.scale(Eigen::Vector3d(1.0, 1.0, x_axis.length));

        Eigen::Vector4d trans = 0.5 * (x_axis.start_position + x_axis.end_position);
        Eigen::Affine3d model = x_axis.global_facing;
        model.prerotate(x_axis.rotation.rotation()).pretranslate(trans.head<3>());
        axis_graphics.setModelMatrix(model.cast<float>());
        axis_graphics.render(&renderProgram);*/
...
void mainPanel(kinematics::Ball* ball) {
    ...
    if (ImGui::Begin("Control")) {
        ...
        Eigen::Vector4f ballPosition(bpos[0], bpos[1], bpos[2], bpos[3]);

        

        if (ImGui::SliderFloat("target x", &ballPosition[0], -10.0f, 10.0f)) {
            // ImGui::InputDouble("target x", &bpos[0], 0.01, 0.1, "%.2lf")
            //bpos[0] = std::clamp(bpos[0], -10.0, 10.0);
            bpos[0] = ballPosition[0];
        }
        if (ImGui::SliderFloat("target y", &ballPosition[1], -10.0f, 10.0f)) {
            /*ImGui::InputDouble("target y", &bpos[1], 0.01, 0.1, "%.2lf")
            bpos[1] = std::clamp(bpos[1], -10.0, 10.0);*/
            bpos[1] = ballPosition[1];
        }
        if (ImGui::SliderFloat("target z", &ballPosition[2], -10.0f, 10.0f)) {
            /*ImGui::InputDouble("target z", &bpos[2], 0.01, 0.1, "%.2lf")
            bpos[2] = std::clamp(bpos[2], -10.0, 10.0);*/
            bpos[2] = ballPosition[2];
        }
