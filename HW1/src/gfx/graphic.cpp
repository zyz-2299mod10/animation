...
std::vector<GLfloat> Bowl::vertices;
std::vector<GLuint> Bowl::fillIndices = std::vector<GLuint>();
std::vector<GLuint> Bowl::wireIndices = std::vector<GLuint>();
...
void Bowl::setRenderMode(RenderMode _mode) { mode = _mode; }

void Bowl::generateVertices() {
    ...
    constexpr float stackStep = util::PI<float>()/4 / g_SphereStacks;
    float sectorAngle, stackAngle;

    for (int i = 0; i <= g_SphereStacks; ++i) {
        //stackAngle = util::PI<float>() / 2.0f - i * stackStep;  // [pi/2, -pi/2] 
        stackAngle = -util::PI<float>() / 4.0f  - i * stackStep;  // [-pi/4, -pi/2] 
    ...
    for (int i = 0; i < g_SphereStacks; ++i) {
        ...
        for (int j = 0; j < g_SphereSectors; ++j, ++k1, ++k2) {
            ...
            if (i != 0) {
                ...
                fillIndices.insert(fillIndices.end(), {k1 + 1, k2, k1});
            }
            // k1+1 => k2 => k2+1
            //if (i != (g_SphereStacks - 1)) {
            //    // fillIndices.insert(fillIndices.end(), {k1 + 1, k2, k2 + 1});
            //    fillIndices.insert(fillIndices.end(), {k1 + 1, k2, k2 + 1});
            //    fillIndices.insert(fillIndices.end(), {k2 + 1, k2, k1 + 1});
            //}
            if (i != (g_SphereStacks - 1)) {
                // fillIndices.insert(fillIndices.end(), {k1 + 1, k2, k2 + 1});
                fillIndices.insert(fillIndices.end(), {k2 + 1, k2, k1 + 1});
            ...
            if (i == 0) {
                // fillIndices.insert(fillIndices.end(), {k1 + 1, k1, k2});
                fillIndices.insert(fillIndices.end(), {k2, k1, k1 + 1});
            }
        }
    }
}

void Bowl::render(Program* shaderProgram) {
    if (texture) {
        shaderProgram->setUniform("useTexture", 1);
        shaderProgram->setUniform("diffuseTexture", texture->getIndex());
    } else {
    ...
    shaderProgram->setUniform("obj", 1);
    shaderProgram->setUniform("model", modelMatrix);
    shaderProgram->setUniform("invtransmodel", inverseTransposeModel);
