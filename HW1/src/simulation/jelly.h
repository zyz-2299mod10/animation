...
class Jelly {
 public:
    Jelly();
    Jelly(const Eigen::Vector3f &a_kInitPos, const float jellyLength, const int numAtEdge, const float dSpringCoef,
    ...
    void resetJelly(const Eigen::Vector3f &offset, const float &rotate);
    // add gravity
