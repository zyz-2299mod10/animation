...
constexpr const char* studentID = "109350008";
...
// Current terrain
gfx::Rigidbody* BowlTerrainGraphics = nullptr;
gfx::Rigidbody* PlaneTerrainGraphics = nullptr;
// Plane and tiltedplane share same vertices
// Their difference is model matrix
std::unique_ptr<gfx::Plane> g_Plane = nullptr;
// Bowl and wirebowl share same vertices
// But they have different render behavior based on
// gfx::Bowl::RenderMode::WIREFRAME or gfx::Bowl::RenderMode::FILLED
std::unique_ptr<gfx::Bowl> g_Bowl = nullptr;
// Soft body jelly's graphic
std::unique_ptr<gfx::SoftJelly> g_jelly = nullptr;
...
int main() {
    ...
    g_jelly = std::make_unique<gfx::SoftJelly>(particleSystem.getJellyPointer(0));
    ...
    // Load data from assets
    {
        ...
        auto skyboxFileList = std::array<util::fs::path, 6>(
            {pf::find("Texture/ocean0.png"), pf::find("Texture/ocean1.png"), pf::find("Texture/ocean2.png"),
             pf::find("Texture/ocean3.png"), pf::find("Texture/ocean4.png"), pf::find("Texture/ocean5.png")});
        ...
        g_jelly->setTextures({jelly1, jelly2, jelly3, jelly4, jelly5, jelly6});
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (particleSystem.isSimulating) g_jelly->update();
        ...
        BowlTerrainGraphics->render(&shadowProgram);
        PlaneTerrainGraphics->render(&shadowProgram);

        // Rendor jelly's shadow
        if (particleSystem.isDrawingJelly)
            g_jelly->renderJelly(&shadowProgram);
        else
            g_jelly->renderPoints(&shadowProgram);

        if (particleSystem.isDrawingStruct) {
            g_jelly->renderLines(&shadowProgram, simulation::Spring::SpringType::STRUCT);
        }
        if (particleSystem.isDrawingShear) {
            g_jelly->renderLines(&shadowProgram, simulation::Spring::SpringType::SHEAR);
        }
        if (particleSystem.isDrawingBending) {
            g_jelly->renderLines(&shadowProgram, simulation::Spring::SpringType::BENDING);
        }
        ...
        if (!particleSystem.isDrawingJelly) {
            g_jelly->renderPoints(&simpleRenderProgram);
        }
        if (particleSystem.isDrawingStruct) {
            g_jelly->renderLines(&simpleRenderProgram, simulation::Spring::SpringType::STRUCT);

        }
        if (particleSystem.isDrawingShear) {

            g_jelly->renderLines(&simpleRenderProgram, simulation::Spring::SpringType::SHEAR);
        }
        if (particleSystem.isDrawingBending) {
            g_jelly->renderLines(&simpleRenderProgram, simulation::Spring::SpringType::BENDING);
        }
        // 2b. Render scene (jelly / terrain)
        renderProgram.use();
        renderProgram.setUniform("viewPos", currentCamera->getPosition());
        renderProgram.setUniform("VP", currentCamera->viewWithProjection());
        BowlTerrainGraphics->render(&renderProgram);
        PlaneTerrainGraphics->render(&renderProgram);
        if (particleSystem.isDrawingJelly) {
            const bool jellyWithLine =
            ...
            g_jelly->renderJelly(&renderProgram);
...
GLFWwindow* initialize() {
    ...
    auto ivory = std::make_shared<gfx::Texture>(util::PathFinder::find("Texture/ivory.png"));
    g_Plane = std::make_unique<gfx::Plane>();
    g_Plane->setTexture(wood);
    g_Bowl = std::make_unique<gfx::Bowl>();
    g_Bowl->setTexture(wood);
    // Physics engine
    auto BowlTerrain = simulation::TerrainFactory::CreateTerrain(simulation::TerrainType::Bowl);
    auto PlaneTerrain = simulation::TerrainFactory::CreateTerrain(simulation::TerrainType::Plane);
    ...
    g_Bowl->setRenderMode(gfx::Bowl::RenderMode::FILLED);
    BowlTerrainGraphics = g_Bowl.get();
    BowlTerrainGraphics->setModelMatrix(BowlTerrain->getModelMatrix());
    PlaneTerrainGraphics = g_Plane.get();
    PlaneTerrainGraphics->setModelMatrix(PlaneTerrain->getModelMatrix());
    particleSystem.setTerrain(std::move(BowlTerrain), "BOWL");
    particleSystem.setTerrain(std::move(PlaneTerrain), "PLANE");
    return window;
}
...
void mainPanel() {
    // Main Panel
    ImGui::SetNextWindowSize(ImVec2(220.0f, 125.0f), ImGuiCond_Once);
...
void recordingPanel() {
    ...
    ImGui::SetNextWindowCollapsed(0, ImGuiCond_Once);
    ImGui::SetNextWindowPos(ImVec2(60.0f, 190.0f), ImGuiCond_Once);
    ImGui::SetNextWindowBgAlpha(0.2f);
...
void simulationPanel() {
    ...
    if (ImGui::Begin("Simulation Control Panel", &isUsingSimulationPanel)) {
        ...
        ImGui::Checkbox("Draw Jelly", &particleSystem.isDrawingJelly);
        ...
        if (particleSystem.isSimulating) {
            ...
            if (ImGui::InputFloat("Jelly Y Offset", &particleSystem.position[1], 0.3f, 0.05f)) {
                particleSystem.position[1] = std::max(particleSystem.position[1], 1.0f);
            }
            ImGui::InputFloat("Jelly Rotation", &particleSystem.rotation, 0.3f, 0.05f);
        }
        if (ImGui::Button("Reset Jelly")) {
            particleSystem.reset();
            g_jelly->update();
        }
    }
