...
bool isUsingWarpingPanel = false;
...
int pre_keyframe = 328;
int post_keyframe = 328;
bool resetTimeWarp = false;
// Fonts' range
...
int main() {
    ...
    acclaim::Motion running(acclaim_folder / "walk.amc", std::make_unique<acclaim::Skeleton>(*skeleton));
    acclaim::Motion shooting(acclaim_folder / "shoot.amc", std::move(skeleton));
    shooting.getSkeleton()->setBoneColor(Eigen::Vector4f(0.12f, 0.28f, 0.53f, 0.0f));
    
    shooting.timeWarper(pre_keyframe, post_keyframe);
    ...
    kinematics::Ball ball(shooting.getSkeleton()->getBonePointer(("rfingers")));
    ...
    while (!glfwWindowShouldClose(window)) {
        if (resetTimeWarp) {
            shooting.timeWarper(pre_keyframe, post_keyframe);
            ball.set_thrown_frame(post_keyframe);
            resetTimeWarp = false;
        }
        if (isTimeWarping)
        ...
        if (isTimeWarping) {
            shooting.setBoneTransform(currentFrame);
        ...
        if (isTimeWarping) {
            ...
            shooting.getSkeleton()->render(&shadowProgram);
        } else {
        ...
        if (isTimeWarping) {
            ...
            shooting.getSkeleton()->render(&renderProgram);
        ...
        // 4. Render ImGui UI
        //auto std::make_unique<acclaim::Motion>(punchWarped)
...
GLFWwindow* initialize() {
    ...
    // Create GLFW context
    const auto screenTitle = std::string("Forward Kinematics");
...
void mainPanel(int* frame, int maxFrame) {
    ...
    if (ImGui::Begin(ICON_CODE " Frame Control")) {
        ...
        ImGui::SameLine();
        if (ImGui::Button("Warping Control")) {
            isUsingWarpingPanel ^= true;
        }
...
void warpingPanel(GLFWwindow* window, int* frame) {
    ImGui::SetNextWindowSize(ImVec2(500.0f, 95.0f), ImGuiCond_Once);
    ImGui::SetNextWindowCollapsed(0, ImGuiCond_Once);
    ImGui::SetNextWindowPos(ImVec2(250.0f, 540.0f), ImGuiCond_Once);
    ImGui::SetNextWindowBgAlpha(0.2f);
    if (ImGui::Begin("Warping Control", &isUsingWarpingPanel)) {
        if (!isUsingFreeCamera) ImGui::Text("Use this panel to control the warping parameter");
        ImGui::SliderInt("Post Keyframe", &post_keyframe, 100, 600);
        if (ImGui::Button("Set")) {
            *frame = 0;
            resetTimeWarp = true;
        }
...
void cameraPanel(GLFWwindow* window) {
    // Camera control
    // Create imgui window
    ImGui::SetNextWindowSize(ImVec2(500.0f, 180.0f), ImGuiCond_Once);
    ImGui::SetNextWindowCollapsed(0, ImGuiCond_Once);
    ImGui::SetNextWindowPos(ImVec2(250.0f, 355.0f), ImGuiCond_Once);
    ImGui::SetNextWindowBgAlpha(0.2f);
...
void renderUI(GLFWwindow* window, int* frame, int maxFrame) {
    ...
    if (isUsingWarpingPanel) warpingPanel(window, frame);
    ImGui::Render();
